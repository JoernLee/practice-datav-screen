...
<script>
  ...
  export default {
    ...
    setup () {
      ...
      const update = () => {
        options.value = {
          // baseOption配置一些静态的，不随时间节点变化的通用配置
          baseOption: {
            timeline: {
              axisType: 'category',
              autoPlay: true,
            ...
            title: {
              // subTitle写死的，main title是动态配置的，在后面的options里面
              // 当移动到对应时间节点时，会将两种option进行merge，所以title就全了
              subtext: '数据来自国家统计局'
            },
            tooltip: {},
            legend: {
            ...
            xAxis: [{
              type: 'category',
              axisLabel: {
                // 这里interval一般配置为auto，但是这里为了让所有label都显示出来（即便重叠拥挤）配置为了0
                interval: 0
              },
              // 这里可以注意一下，因为城市比较多，所以每隔一个添加了换行符，减少拥挤情况
              data: ['北京', '\n天津', '河北', '\n山西', '内蒙古', '\n辽宁', '吉林', '\n黑龙江', '上海', '\n江苏', '浙江', '\n安徽', '福建', '\n江西', '山东', '\n河南', '湖北', '\n湖南', '广东', '\n广西', '海南', '\n重庆', '四川', '\n贵州', '云南', '\n西藏', '陕西', '\n甘肃', '青海', '\n宁夏', '新疆'],
              splitLine: {
                show: false
              }
            ...
            // 通过series可以比较清楚了解当前页面绘制了哪些图形
            // 因为legend只配置了三个bar，所以没展示全
            series: [{
              name: 'GDP',
              type: 'bar'
            }, {
          ...
          // options是根据timeline的节点值动态变化的内容，所以数组length对应timeline节点数
          options: [{
            // 内部的series对应baseOption配置的数据组数，最后的是饼图数据
            title: {
              text: '2002全国宏观经济指标'
            },
            series: [{
