    ...
    <div class="item-wrapper">
      ...
      >
        <div class="left-part">
          <div class="bg">
            <div class="img">
              <icon :name="iconData[index]" :style="{color: 'white', width: '6vw'}"/>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    setup (props) {
      ...
      const iconData = ref(['jiage', 'orders', 'icon-test', 'user'])
      const updateData = (data, oldProps, newProps) => {
        project.value = [{
          title: '转化率',
      ...
      watch(() => props.data, (newProps, oldProps) => {
        updateData(props.data, oldProps, newProps)
      })
      onMounted(() => {
        updateData(props.data, props.data, props.data)
      })
      return {
        project,
        headerData,
